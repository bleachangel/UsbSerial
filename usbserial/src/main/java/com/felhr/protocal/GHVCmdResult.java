...
public class GHVCmdResult extends ProtocalCmd {
    public int mLen;
	public byte[] mHardwareVersion;

	//len(1)+session_id(2)+ version size(1) +firmware version(<=32)+crc(2)+index(1)
    public GHVCmdResult(int cmd, byte[] para){
        super();
        mCmdValue = cmd;
        mParaLen = (int)para[0];
        if(Array.getLength(para) == mParaLen + 1 && mParaLen > 0) {
            ...
            if(mCRC == crc) {
                ...
                mLen = para[3];
                if(mLen > 0) {
                    mHardwareVersion = new byte[mLen];
                    System.arraycopy(para, 4, mHardwareVersion, 0, mLen);
                }
            }
        } else {
            mValid = false;
